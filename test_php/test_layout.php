...
<?php include_once './topContent.php';?>
...
<!-- Fieldsets -->
<fieldset id="first" class="MSF w3-card-2">
<legend>Enter a Gene Symbol</legend>
...
<fieldset class="MSF w3-card-2">
<legend>Choose an Analysis</legend>
...
<fieldset class="MSF w3-card-2">
<legend>Pick a TCGA cohort</legend>
...
<fieldset class="MSF w3-card-2">
<legend>Select Samples and Method</legend>
...
<div class="result">
<div id="CoexpressionAnalysisResult" class="w3-card-2 hide"></div>
<div id="CoexpFollowUp" class="w3-card-2 hide MSF">
...
//The following lines are to clear values from progress bar
function clearGene(){
	grace.gene = '';
...
function setCohortOptions(){
	...
	if(grace.analysis==='Co-expression Analysis'){
		...
		$(".CA-only").hide();
		$(".not-CA-only").show();
	/* Script here to exclude LAML and MESO */
...
function reset(){
	...
	$("#EnrichmentOutput").removeClass("w3-card-2");
	grace.length = '100';
	grace.order = 'asc';
	grace.setDB = '0';
	grace.setID = '-1';
...
function generateScatterPlot(){
	...
	var commonData = [];
	var RNAindex;
	var urlRNA="http://firebrowse.org/api/v1/Samples/mRNASeq?format=json&gene="+grace.gene+"&cohort="+grace.cohort+"&sample_type=TP&protocol=RSEM&page_size=2000&sort_by=tcga_participant_barcode";
	var urlCN="http://firebrowse.org/api/v1/Analyses/CopyNumber/Genes/All?format=json&cohort="+grace.cohort+"&gene="+grace.gene+"&page_size=2000&sort_by=tcga_participant_barcode";
